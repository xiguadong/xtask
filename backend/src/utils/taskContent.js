...
export function readTaskDescriptionContent(projectPath, task) {
  ...
  if (task.description_file) {
    return readFile(projectPath, task.description_file) || task.description || '';
  }
  return task.description || '';
}

export function readTaskSummaryContent(projectPath, task) {
  if (!task?.summary_file) return '';
  return readFile(projectPath, task.summary_file) || '';
}
