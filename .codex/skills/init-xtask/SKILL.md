...
- 初始化时优先检查仓库根目录 `AGENTS.md`，把 `xtask-safe` 规则写入其中
- 如果仓库还没有 `AGENTS.md`，必须先征求用户是否创建；只有用户同意后才创建并写入规则
...
### 4. 处理 `AGENTS.md` 与 `xtask-safe` 规则

初始化仓库时，还要检查仓库根目录是否已有 `AGENTS.md`：

- 如果已有 `AGENTS.md`，直接把 `xtask-safe` 规则补充进去
- 如果没有 `AGENTS.md`，先用一句中文问用户是否允许创建
- 只有在用户明确同意后，才创建 `AGENTS.md` 并写入规则
...
```text
当前仓库还没有 AGENTS.md。是否需要我帮你创建一个，并写入 xtask-safe 的任务维护规则？
```

如果仓库里已经有 `AGENTS.md`，不要再额外征求“是否写入”的确认，直接补齐规则即可。
...
- 是否发现并迁移旧 `.xtask/`
- 是否已补充或创建 `AGENTS.md`
- 如果没创建 `AGENTS.md`，是因为用户拒绝还是仓库已跳过
