...
xtask task create "子任务标题" \
  ...
  --labels <父任务的labels>,parent:<parent-id>,agent:codex
```
...
```bash
xtask task create "子任务1" --milestone <M> --labels <继承>,parent:<parent-id>,agent:codex
xtask task create "子任务2" --milestone <M> --labels <继承>,parent:<parent-id>,agent:codex
```
...
xtask task create "实现用户认证API" \
  --milestone M1 \
  --labels backend,api,parent:T-001,agent:codex
```
...
# 2. 创建子任务
xtask task create "设计数据库schema" --milestone M1 --labels backend,api,parent:T-001,agent:codex
xtask task create "实现API端点" --milestone M1 --labels backend,api,parent:T-001,agent:codex
xtask task create "编写单元测试" --milestone M1 --labels backend,api,parent:T-001,agent:codex
```
