...
├── e2e/                    # Playwright 验收测试
├── design-system/
│   ├── xtask/              # UI 设计系统（MASTER.md + pages/）
│   └── design_v1/          # 技术设计文档（架构、数据模型、API、实施计划）
└── prd_discussion/         # PRD 讨论文档
```

## 数据存储

- 文件化存储：每个项目 `<project_dir>/.agents/task_graph.yaml`
...
- 实现首页 → 先读 `design-system/xtask/pages/home.md`，未覆盖项继承 `MASTER.md`
- 实现子页 → 先读 `design-system/xtask/pages/project-detail.md`，未覆盖项继承 `MASTER.md`
...
- 详见 `design-system/design_v1/api-spec.md`
...
| `design-system/design_v1/` | 技术设计文档 | 架构变更时 |
| `design-system/xtask/` | UI 设计系统 | 视觉规范变更时 |
| `prd_discussion/` | PRD 讨论记录 | 需求变更时 |
