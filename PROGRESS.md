...
- ✅ “删除任务”移入左侧信息栏，降低危险操作与常规配置混排造成的误触风险
- ✅ Worktree 创建/关联操作移入右侧栏，阅读任务内容时仍可并行处理分支与终端
- ✅ 修复网页新建中文标题任务兼容性：任务标题继续支持中文输入与展示，但底层任务 ID / 推荐分支名统一改为 ASCII 安全后缀，避免 Unicode 路径在网页链路里造成创建失败
- ✅ 后端 `createTask` 与 CLI `generateTaskId` 统一为同一套 ASCII 安全 ID 规则；纯中文标题会自动生成 `task-<hash>` 后缀，避免再次退化成空 slug
- ✅ 前端任务卡片与详情页继续优先展示中文标题，可读性保留；推荐分支名改为稳定可用的 ASCII 安全名称

## 2026-03-17

- ✅ 任务 ID/分支名生成规则支持中文/Unicode：新建任务默认生成 `YYYY-MM-DD-HH-mm-ss-SSS-任务描述`，同时避免出现缺失描述或难读的 13 位毫秒数分支名
- ✅ 任务详情页把「原始 ID」文案改为「推荐分支名（时间戳-任务描述）」，减少 Worktree 分支命名上的歧义
...
暂无
- ✅ 将技能脚本移入对应 skill 目录并更新 SKILL.md，已完成 (2026-04-07T04:25:01Z)
