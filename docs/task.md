...
### 任务详情页操作区左右分栏（2026-03-18-18-09-59-549-worktree）

- [x] 左侧危险操作 - 将“删除任务”从任务正文操作区移入左侧信息栏，避免与日常配置操作混排 (2026-03-18)
- [x] 右侧 Worktree - 将 Worktree 配置与终端能力集中到右侧栏，阅读任务内容时仍可独立操作 (2026-03-18)
- [x] 布局拆分 - 任务详情页改为左侧元信息 + 中间正文 + 右侧 Worktree/终端三栏结构，保持现有交互不变 (2026-03-18)

### 网页创建中文标题任务兼容修复

- [x] 任务 ID 生成 - 中文标题继续允许输入与展示，但任务 ID / 推荐分支名后缀改为 ASCII 安全 slug，避免网页链路因 Unicode 路径兼容性导致创建失败 (2026-03-18)
- [x] 前后端对齐 - 后端 Web 创建与 CLI 创建统一使用相同的 ASCII 安全 ID 生成规则，纯中文标题自动回退为 `task-<hash>` (2026-03-18)
- [x] 前端展示 - 任务卡片/详情页显示继续优先展示原始中文标题，推荐分支名改为实际可用的 ASCII 安全名称 (2026-03-18)

## 2026-03-17

### 新建任务分支名展示与命名修复（1773732575353-task-description）

- [x] 命名生成 - 任务/分支任务 ID 生成支持中文/Unicode 标题，默认采用 `YYYY-MM-DD-HH-mm-ss-SSS-任务描述`，避免出现缺失描述或难读的 13 位毫秒数分支名 (2026-03-17)
- [x] 页面展示 - 任务详情页将「原始 ID」改为「推荐分支名（时间戳-任务描述）」，并沿用新的可读时间戳格式 (2026-03-17)
